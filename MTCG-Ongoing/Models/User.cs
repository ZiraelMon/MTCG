...
namespace MTCG.Models {
    public class User {
        ...
        public void GetCarddeck(HttpSvrEventArgs e, Token userToken) {
            ...
            try {
                ...
                if (coins < 5) {
                    e.Reply(400, "Funds too low.");
                    return;
                }

                // Retrieve first carddeck from the database
                using (var cmd = new NpgsqlCommand("SELECT pid, card1, card2, card3, card4, card5 FROM carddeck ORDER BY pid ASC LIMIT 1", conn))
                using (var reader = cmd.ExecuteReader()) {
                    if (reader.Read()) {
                ...
                using (var cmd = new NpgsqlCommand("DELETE FROM carddeck WHERE pid = @p1", conn)) {
